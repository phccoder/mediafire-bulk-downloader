...
def download_file(
    ...
    # Try to get total size from file info dictionary
    ...
    if path.exists(filename):
        if hash_file(filename) == file["hash"]:
            ...
            # --- GUI SIGNAL ---
            if 'UI_CALLBACK' in globals():
                UI_CALLBACK(filename, total_size, total_size, 0, "Skipped (Exists)")
            if limiter:
                limiter.release()
            return
    ...
    # Handle Mediafire's GZip / Scrambled links
    if response.getheader("Content-Encoding") == "gzip":
        ...
        with GzipFile(fileobj=BytesIO(compressed_data)) as f:
            ...
            if "data-scrambled-url" in download_btn.attrs:
                base64_data = download_btn.attrs["data-scrambled-url"]
                decode_base_64_link = base64.b64decode(base64_data).decode("utf-8")
            else:
                decode_base_64_link = download_btn.attrs["href"]
    ...
    if 400 <= response.status < 600:
        conn.close()
        if 'UI_CALLBACK' in globals():
            UI_CALLBACK(filename, 0, total_size, 0, "Error")
        if limiter:
            limiter.release()
        return

    # Fallback to get size from header if it wasn't in metadata
    if total_size == 0 and response.getheader("Content-Length"):
    ...
    with open(filename, "wb") as f:
        while True:
            chunk = response.read(8192) # 8KB chunks for smoother GUI progress
            ...
            if event and event.is_set():
                ...
                remove(filename)
                if 'UI_CALLBACK' in globals():
                    UI_CALLBACK(filename, downloaded_size, total_size, 0, "Cancelled")
                if limiter:
                    limiter.release()
                return
            ...
            # --- GUI SIGNAL ---
            # Update UI periodically (every 0.2s) to prevent the app from freezing
            current_time = time.time()
            if 'UI_CALLBACK' in globals() and (current_time - last_ui_update > 0.2):
                elapsed = current_time - start_time
                speed = downloaded_size / elapsed if elapsed > 0 else 0
                UI_CALLBACK(filename, downloaded_size, total_size, speed, "Downloading...")
    ...
    if 'UI_CALLBACK' in globals():
        ...
        speed = downloaded_size / elapsed if elapsed > 0 else 0
        UI_CALLBACK(filename, total_size, total_size, speed, "Completed")
